      ...
      <main className="min-h-screen flex items-center justify-center p-6">
        <div className="w-full max-w-md border rounded-lg p-6 bg-dark-panel shadow text-foreground text-center">
          <h1 className="text-xl font-semibold mb-4">
            Welcome to your business OS!
          </h1>
          <div className="text-muted mb-2">
            Taking you to your daily dashboard…
          </div>
          <button
    ...
    <main className="min-h-screen flex items-center justify-center p-6">
      <div className="w-full max-w-md border rounded-lg p-6 bg-dark-panel shadow text-foreground">
        ...
        <p className="mb-4 text-muted">
          You’re running more than one job — most contractors upgrade here to
          stay organized.
        </p>
        <div className="mb-6">
          ...
          <ul className="list-disc ml-6 text-sm">
            {PLAN_FEATURES["Scale"].map((f) => (
              <li key={f} className="flex items-center">
                {f}
                <InfoTooltip
                  text={`Feature: ${f}. Learn more in our help center.`}
                />
              </li>
            ))}
          </ul>
        ...
        <div className="mb-6">
          <div className="font-semibold mb-1">
            Your current plan:{" "}
            <span className="text-accent">{currentPlan}</span>
          </div>
          <div className="text-xs text-muted">
            Upgrade to Scale or Pro for more power as you grow.
          </div>
        </div>
        <div className="mb-4">
          <button
          ...
          {workspaceId && <UpgradeButton workspaceId={workspaceId} />}
          <button
            className="w-full mt-2 border border-muted text-muted bg-background rounded-md p-2 font-medium"
            onClick={() =>
              alert("Draft saved! You can return and upgrade later.")
            }
          ...
          {!workspaceId && (
            <div className="text-red-500 text-xs mt-2 text-center">
              Workspace ID not found. Please reload the page or contact support.
              <br />
              <span className="text-muted">
                (You can also check your workspace selection in the main app.)
              </span>
            </div>
          )}
          {upgradeError && (
        ...
        <div className="text-xs text-muted mt-3 text-center flex flex-col items-center">
          <span>
            No hard block — you can keep working, but upgrading unlocks more
            features.
          </span>
          <span className="mt-2">
            Your privacy is protected.{" "}
            <InfoTooltip text="We never share your business data with third parties. Billing info is encrypted and handled securely." />
          </span>
        </div>
      </div>
    </main>
