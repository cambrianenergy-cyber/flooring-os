...
export default function RoomEstimateDemo() {
  ...
  return (
    <div className="max-w-md mx-auto p-6 border rounded space-y-4 bg-background text-slate-900">
      ...
      <div className="space-y-2">
        ...
        <label className="block">
          ...
          <input
            ...
            onChange={(e) => setLaborMultiplier(Number(e.target.value))}
          />
        </label>
      </div>
      <div className="mt-4 p-4 bg-neutral-50 rounded">
        <div>
          Material Cost:{" "}
          <span className="font-mono">${estimate.materialCost.toFixed(2)}</span>
        </div>
        <div>
          Labor Cost:{" "}
          <span className="font-mono">${estimate.laborCost.toFixed(2)}</span>
        </div>
        <div className="font-bold">
          Total: <span className="font-mono">${estimate.total.toFixed(2)}</span>
        </div>
      </div>
    </div>
  );
