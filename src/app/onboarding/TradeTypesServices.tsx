...
export default function TradeTypesServices() {
  ...
  const laborCategorySuggestions = [
    "Installer",
    "Helper",
    "Supervisor",
    "Estimator",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    // Optional fields
    const flooringList = flooringTypes
      ? flooringTypes.split(/[,s]+/).filter(Boolean)
      : null;
    const addOnList = addOns ? addOns.split(/[,s]+/).filter(Boolean) : null;
    const laborList = laborCategories
      ? laborCategories.split(/[,s]+/).filter(Boolean)
      : null;
    try {
  ...
  async function handleSaveDraft() {
    ...
    try {
      ...
      if (onboardingRef) {
        await updateDoc(onboardingRef, {
          "data.flooringTypes": flooringTypes
            ? flooringTypes.split(/[,\s]+/).filter(Boolean)
            : null,
          "data.addOns": addOns ? addOns.split(/[,\s]+/).filter(Boolean) : null,
          "data.laborCategories": laborCategories
            ? laborCategories.split(/[,\s]+/).filter(Boolean)
            : null,
          updatedAt: serverTimestamp(),
        });
  ...
  return (
    ...
    >
      <form
        ...
        aria-label="Trade Types & Services Form"
      >
        <OnboardingProgress currentStep={5} />
        <h1 className="text-xl font-semibold mb-4 text-foreground">
          Trade Types & Services
        </h1>
        <div className="flex items-center mt-2">
          <label htmlFor="flooringTypes" className="block text-sm">
            Flooring Types (comma or space separated)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="List all flooring types you offer, separated by commas or spaces. Click for suggestions."
            onClick={() => setShowFlooringSuggestions(!showFlooringSuggestions)}
          >
            ?
          </span>
        </div>
          ...
          <ul className="mt-1 text-xs bg-muted rounded p-2">
            {flooringTypeSuggestions.map((f) => (
              <li
                ...
                onClick={() => {
                  setFlooringTypes(
                    flooringTypes ? flooringTypes + ", " + f : f,
                  );
                  setShowFlooringSuggestions(false);
                }}
              >
                {f}
              </li>
        ...
        <div className="flex gap-2 mt-1">
          {flooringTypeSuggestions.map((f) => (
            <span
              ...
              onClick={() =>
                setFlooringTypes(flooringTypes ? flooringTypes + ", " + f : f)
              }
            >
              {f}
            </span>
        ...
        <div className="flex items-center mt-3">
          <label htmlFor="addOns" className="block text-sm">
            Add-Ons (comma or space separated)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="List any additional services you offer, such as Demo or Baseboards. Click for suggestions."
            onClick={() => setShowAddOnSuggestions(!showAddOnSuggestions)}
          >
            ?
          </span>
        </div>
        ...
        <div className="flex gap-2 mt-1">
          {addOnSuggestions.map((a) => (
            <span
              key={a}
              className="px-2 py-1 bg-muted rounded text-xs cursor-pointer"
              onClick={() => setAddOns(addOns ? addOns + ", " + a : a)}
            >
              {a}
            </span>
          ))}
        ...
        <div className="flex items-center mt-3">
          <label htmlFor="laborCategories" className="block text-sm">
            Labor Categories (comma or space separated)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="Specify labor roles, e.g., Installer, Helper. Click for suggestions."
            onClick={() => setShowLaborSuggestions(!showLaborSuggestions)}
          >
            ?
          </span>
        </div>
          ...
          <ul className="mt-1 text-xs bg-muted rounded p-2">
            {laborCategorySuggestions.map((l) => (
              <li
                ...
                onClick={() => {
                  setLaborCategories(
                    laborCategories ? laborCategories + ", " + l : l,
                  );
