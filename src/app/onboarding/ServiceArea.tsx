    ...
    >
      ...
      >
        ...
        <div className="flex items-center mt-2">
          <label htmlFor="zipCodes" className="block text-sm">
            Zip Codes (comma or space separated)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="List all zip codes you serve, separated by commas or spaces. Each must be a 5-digit number. Click for suggestions."
            onClick={() => setShowZipSuggestions(!showZipSuggestions)}
          >
            ?
          </span>
        </div>
          ...
          <ul className="mt-1 text-xs bg-muted rounded p-2">
            {zipSuggestions.map((z) => (
              <li
                ...
                onClick={() => {
                  setZipCodes(zipCodes ? zipCodes + ", " + z : z);
        ...
        <div className="flex items-center mt-3">
          <label htmlFor="radius" className="block text-sm">
            Radius (miles)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="Enter the radius in miles from your main location. You can use the slider below for quick selection."
          >
            ?
          </span>
        </div>
        ...
        <input
          ...
          value={radiusSlider}
          onChange={(e) => {
            setRadiusSlider(Number(e.target.value));
        ...
        <div className="text-xs text-muted-foreground mb-2">
          Selected radius: {radiusSlider} miles
        </div>
        <div className="flex items-center mt-3">
          <label htmlFor="counties" className="block text-sm">
            Counties (comma or space separated)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="List all counties you serve, separated by commas or spaces. Only alphabetic characters allowed. Click for suggestions."
            onClick={() => setShowCountySuggestions(!showCountySuggestions)}
          >
            ?
          </span>
        </div>
          ...
          <ul className="mt-1 text-xs bg-muted rounded p-2">
            {countySuggestions.map((c) => (
              <li
                ...
                className="cursor-pointer hover:text-accent"
                onClick={() => {
                  setCounties(counties ? counties + ", " + c : c);
        ...
        <div className="mt-4 mb-2 w-full h-32 bg-muted flex items-center justify-center rounded border">
          <span className="text-muted-foreground text-xs">
            [Map preview of service area coming soon]
          </span>
        </div>
        {error && (
        ...
        <div className="mt-4 text-xs text-muted" aria-live="polite">
          <span role="note">
            Your service area information is private and only used for
            onboarding.{" "}
            <a
              href="/privacy"
