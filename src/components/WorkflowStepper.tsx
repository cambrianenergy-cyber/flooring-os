  ...
  return (
    <nav className="flex flex-wrap gap-2 my-6 justify-center">
      {steps.map((step, idx) => {
        ...
        return (
          <Link key={step.name} href={step.path} onClick={() => setStep(step.name as any)}>
            <span
              className={`flex items-center px-4 py-2 rounded-full border text-xs font-semibold shadow transition-all
                ${isActive ? "bg-gradient-to-r from-blue-600 to-blue-400 text-white border-blue-600" :
                  isCompleted ? "bg-green-100 text-green-700 border-green-400" :
            ...
            >
              ...
              {isActive && !isCompleted && (
                <button
                  ...
                  onClick={e => { e.preventDefault(); completeStep(step.name as any); }}
                >Mark Done</button>
              )}
            </span>
          </Link>
