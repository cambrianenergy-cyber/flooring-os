...
async function seedDemoCatalog() {
  ...
  for (const item of items) {
    await setDoc(doc(collection(db, "demo_catalog"), item.id), item);
  }
}
...
async function seedAccessoriesCatalog() {
  ...
  for (const item of items) {
    await setDoc(doc(collection(db, "accessories_catalog"), item.id), item);
  }
}
...
async function main() {
  ...
  console.log("Demo and accessories catalogs seeded.");
}
