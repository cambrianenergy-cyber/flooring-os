...
// import { useAuth } from "@/lib/auth"; // <-- Add this import, adjust path if needed
...
export default function CatalogMaterials() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const productList = products
      ? products.split(/[,\s]+/).filter(Boolean)
      : null;
    const supplierList = supplierLinks
      ? supplierLinks.split(/[,\s]+/).filter(Boolean)
      : null;
    try {
      ...
      await logOnboardingEvent("saved", 9, {
        products: productList,
        supplierLinks: supplierList,
      });
    ...
    <main className="min-h-screen flex items-center justify-center p-6">
      ...
      >
        ...
        <label className="block mt-2 text-sm">
          Top Products (comma or space separated)
        </label>
        <div className="flex items-center mt-2">
          <label htmlFor="products" className="block text-sm">
            Top Products (comma or space separated)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="List your top products. Click for suggestions."
          ...
          <ul className="mt-1 text-xs bg-muted rounded p-2">
            {productSuggestions.map((p) => (
              <li
                ...
                onClick={() => {
                  setProducts(products ? products + ", " + p : p);
        ...
        <div className="flex gap-2 mt-1">
          {productSuggestions.map((p) => (
            <span
              ...
              onClick={() => setProducts(products ? products + ", " + p : p)}
            >
              {p}
            </span>
          ))}
        ...
        <label className="block mt-3 text-sm">
          Supplier Links (comma or space separated)
        </label>
        <div className="flex items-center mt-3">
          <label htmlFor="supplierLinks" className="block text-sm">
            Supplier Links (comma or space separated)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="Paste links to your suppliers. Click for suggestions."
        ...
        <input
          ...
          className="w-full border rounded-md p-2 mt-1"
          value={supplierLinks}
          onChange={(e) => setSupplierLinks(e.target.value)}
          placeholder="https://supplier1.com, https://supplier2.com"
          aria-label="Supplier Links"
        />
          ...
          <ul className="mt-1 text-xs bg-muted rounded p-2">
            {supplierSuggestions.map((s) => (
              <li
                ...
                onClick={() => {
                  setSupplierLinks(
                    supplierLinks ? supplierLinks + ", " + s : s,
                  );
        ...
        <div className="flex gap-2 mt-1">
          {supplierSuggestions.map((s) => (
            <span
              ...
              className="px-2 py-1 bg-muted rounded text-xs cursor-pointer"
              onClick={() =>
                setSupplierLinks(supplierLinks ? supplierLinks + ", " + s : s)
              }
            >
              {s}
            </span>
        ...
        {supplierLinks &&
          supplierLinks.split(/[\,\s]+/).some((link) => !isValidUrl(link)) && (
            <div className="text-xs text-danger mt-1">
              Please enter valid URLs for all supplier links.
            </div>
          )}
        {error && (
                ...
                if (onboardingRef) {
                  transaction.update(onboardingRef, {
                    "data.products": products
                      ? products.split(/[\,\s]+/).filter(Boolean)
                      : null,
                    "data.supplierLinks": supplierLinks
                      ? supplierLinks.split(/[\,\s]+/).filter(Boolean)
                      : null,
                    updatedAt: serverTimestamp(),
                  });
        ...
        <div className="mt-4 text-xs text-muted-foreground" aria-live="polite">
          <span role="note">
            Your catalog/materials info is private and only used for onboarding.{" "}
            <a
              href="/privacy"
