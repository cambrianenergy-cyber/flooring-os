    ...
    <button
      ...
      className={[
        ...
        active
          ...
          : "border-slate-200 bg-background text-slate-900 hover:bg-slate-50 text-slate-900",
      ].join(" ")}
    >
  ...
  return (
    <OnboardingShell step={5}>
      <FormCard
        ...
        subtitle="Pick your default operating system. You can install more later."
      >
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {PACKS.map((p) => (
            <PackCard
              key={p.key}
              pack={p}
              active={selectedPack === p.key}
              onSelect={() => dispatchPack({ type: "set", value: p.key })}
            />
          ))}
        </div>

        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-slate-50 text-slate-900 p-4">
          <div>
