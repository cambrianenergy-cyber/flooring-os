...
export function AIUsageBanner({
  ...
  if (percent >= 1) {
    if (isElite) {
      ...
      cta = (
        <button className="ml-2 bg-accent text-background rounded px-3 py-1 font-bold">
          Buy Overage
        </button>
      );
      color = "bg-red-900";
  ...
  return (
    <div
      className={`rounded-lg p-4 mb-4 text-background flex flex-col md:flex-row items-center justify-between ${color}`}
    >
      <div>
        <div className="font-bold text-lg mb-1">AI Usage</div>
        <div className="mb-1">
          {Math.round(percent * 100)}% used ({used.toLocaleString()} /{" "}
        ...
        {topFeature && (
          <div className="mb-1">
            Biggest drain:{" "}
            <span className="font-semibold">{topFeature.featureKey}</span> (
            {topFeature.count} runs)
          </div>
        )}
        <div className="text-xs text-muted">
          No surprise: usage resets monthly. You’ll always see your usage and
