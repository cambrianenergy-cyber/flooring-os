...
export default function DistoFloorPlanEditor({
  roomId,
  onSave,
}: DistoFloorPlanEditorProps) {
  ...
  const snapped = (n: number) =>
    snap ? Math.round(n / gridSize) * gridSize : n;
    ...
    <div className="border rounded p-4 max-w-2xl mx-auto">
      ...
      <div className="flex gap-2 mb-2 items-center">
        <button
          ...
          className={`px-2 py-1 rounded ${mode === "draw" ? "bg-blue-600 text-white" : "bg-gray-200"}`}
        >
          Draw
        </button>
        <button
          onClick={() => setMode("measure")}
          className={`px-2 py-1 rounded ${mode === "measure" ? "bg-blue-600 text-white" : "bg-gray-200"}`}
        >
          Measure
        </button>
        {/* Undo button */}
        ...
        >
          <svg width="18" height="18" viewBox="0 0 20 20" fill="none">
            <path
              d="M9 17C13.4183 17 17 13.4183 17 9C17 4.58172 13.4183 1 9 1C6.23858 1 3.82843 2.34315 2.34315 4.34315M2 7V4.5C2 3.67157 2.67157 3 3.5 3H6"
              stroke="#222"
              strokeWidth="2"
              strokeLinecap="round"
        ...
        <button
          ...
          className={`px-2 py-1 rounded ${distoConnected ? "bg-green-600 text-white" : "bg-yellow-500 text-foreground"} cursor-pointer`}
        ...
        {distoError && (
          <span className="text-red-600 text-xs ml-2">{distoError}</span>
        )}
        {/* ...existing controls... */}
        ...
        <div className="relative">
          ...
          {showDropdown && (
            <div className="absolute left-0 mt-2 w-48 bg-white text-slate-900 border rounded shadow-lg z-10 p-3 flex flex-col gap-2">
        ...
        {ACCESSORY_LIST.map((acc) => (
          ...
          >
            <input
              ...
              onChange={(e) =>
                setAccessories((a) =>
                  e.target.checked ? [...a, acc] : a.filter((x) => x !== acc),
                )
              }
            />
            {acc}
        ...
        <svg width={400} height={400} className="absolute top-0 left-0">
          {points.map((pt, i) => (
            <circle
              ...
              cy={pt.y}
              r={5}
              fill={selectedIdx === i ? "#f59e42" : "#2563eb"}
              onClick={(e) => {
                e.stopPropagation();
                handleEditPoint(i);
              }}
      ...
      {editing && selectedIdx != null && (
        <div className="mb-2 flex gap-2 items-center">
          ...
          <button onClick={() => handleMovePoint(-10, 0)} className="px-2">
            ←
          </button>
          <button onClick={() => handleMovePoint(10, 0)} className="px-2">
            →
          </button>
          <button onClick={() => handleMovePoint(0, -10)} className="px-2">
            ↑
          </button>
          <button onClick={() => handleMovePoint(0, 10)} className="px-2">
            ↓
