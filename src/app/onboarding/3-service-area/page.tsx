...
export default function ServiceAreaPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (snap.exists() && snap.data().serviceArea) {
          const { zip, radius } = snap.data().serviceArea;
          if (zip) setZip(zip);
  ...
  return (
    <OnboardingLayout step={3}>
      ...
      <h1 className="text-2xl font-bold mb-4 text-slate-900">Service Area</h1>
      <form
        onSubmit={handleSubmit}
        className="bg-page-surface/80 p-8 rounded-xl shadow w-full max-w-md space-y-4"
      >
        <div>
          <label className="block mb-1 text-slate-700">Zip Code</label>
          <input
        ...
        <div>
          <label className="block mb-1 text-slate-700">Radius (miles)</label>
          <input
            className="w-full border rounded p-2 bg-page-surface text-slate-900"
            value={radius}
        ...
        <div className="flex gap-4 mt-6">
          <button
            ...
            onClick={() => router.push("/onboarding/2-company")}
          >
            Back
          </button>
          <button
