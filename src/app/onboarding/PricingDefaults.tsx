...
export default function PricingDefaults() {
  ...
  const [showLaborRateSuggestions, setShowLaborRateSuggestions] =
    useState(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const safeLaborRate =
      laborRate && isValidNumber(laborRate) ? laborRate : null;
    ...
    try {
      ...
      await logOnboardingEvent("saved", 6, {
  ...
  return (
    ...
    >
      <form
        ...
        aria-label="Pricing Defaults Form"
      >
        <button
          type="button"
        ...
        <div className="flex items-center mt-2">
          <label htmlFor="laborRate" className="block text-sm">
            Labor Rate ($/hr)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="Typical labor rates for flooring contractors. Click for suggestions."
            onClick={() =>
              setShowLaborRateSuggestions(!showLaborRateSuggestions)
            }
          >
            ?
          </span>
          ...
          <ul className="mt-1 text-xs bg-muted rounded p-2">
            {laborRateSuggestions.map((l) => (
              <li
                ...
                onClick={() => {
                  setLaborRate(l);
                  setShowLaborRateSuggestions(false);
                }}
              >{`$${l}/hr`}</li>
            ))}
          </ul>
        ...
        <div className="flex gap-2 mt-1">
          {laborRateSuggestions.map((l) => (
            <span
              ...
              onClick={() => setLaborRate(l)}
            >{`$${l}/hr`}</span>
          ))}
        </div>
        <label className="block mt-3 text-sm">Markup (%)</label>
        <div className="flex items-center mt-3">
          <label htmlFor="markup" className="block text-sm">
            Markup (%)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="Markup is the percentage added to your costs. Click for suggestions."
            onClick={() => setShowMarkupSuggestions(!showMarkupSuggestions)}
          >
            ?
          </span>
        </div>
          ...
          <ul className="mt-1 text-xs bg-muted rounded p-2">
            {markupSuggestions.map((m) => (
              <li
                ...
                onClick={() => {
                  setMarkup(m);
                  setShowMarkupSuggestions(false);
                }}
              >{`${m}%`}</li>
            ))}
          </ul>
        ...
        <div className="flex items-center mt-3">
          <label htmlFor="minJob" className="block text-sm">
            Minimum Job ($)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="Minimum charge for any job. Click for suggestions."
            onClick={() => setShowMinJobSuggestions(!showMinJobSuggestions)}
          >
            ?
          </span>
        </div>
          ...
          <ul className="mt-1 text-xs bg-muted rounded p-2">
            {minJobSuggestions.map((mj) => (
              <li
                ...
                onClick={() => {
                  setMinJob(mj);
                  setShowMinJobSuggestions(false);
                }}
              >{`$${mj}`}</li>
            ))}
          </ul>
        ...
        <div className="flex gap-2 mt-1">
          {minJobSuggestions.map((mj) => (
            <span
              key={mj}
              className="px-2 py-1 bg-muted rounded text-xs cursor-pointer"
              onClick={() => setMinJob(mj)}
            >{`$${mj}`}</span>
          ))}
        </div>
        <label className="block mt-3 text-sm">Trip Fee ($)</label>
        <div className="flex items-center mt-3">
          <label htmlFor="tripFee" className="block text-sm">
            Trip Fee ($)
          </label>
          <span
            className="ml-2 text-xs text-muted cursor-help"
            title="Fee for travel to job site. Click for suggestions."
            onClick={() => setShowTripFeeSuggestions(!showTripFeeSuggestions)}
          >
            ?
          </span>
        </div>
          ...
          <ul className="mt-1 text-xs bg-muted rounded p-2">
            {tripFeeSuggestions.map((tf) => (
              <li
                ...
                onClick={() => {
                  setTripFee(tf);
                  setShowTripFeeSuggestions(false);
                }}
              >{`$${tf}`}</li>
            ))}
          </ul>
        ...
        <div className="mt-4 text-xs text-muted-foreground" aria-live="polite">
          <span role="note">
            Your pricing defaults are private and only used for onboarding.{" "}
            <a
              href="/privacy"
